...
const AddGig: FC = (): ReactElement => {
  ...
  const onCreateGig = async (): Promise<void> => {
    try {
      ...
      // Fallback: Obtener profilePicture de localStorage si Redux no lo tiene
      let profilePicture = authUser.profilePicture;
      if (!profilePicture) {
        try {
          ...
          if (persistRoot) {
            ...
            if (parsed.authUser) {
              const authUserData = JSON.parse(parsed.authUser);
              profilePicture = authUserData.profilePicture;
            }
          }
        } catch (e) {
          console.error('Error getting profile picture from localStorage:', e);
        }
      }
      
      // FIX: Si categories o expectedDelivery tienen valores por defecto, no validar
      // Esto es un workaround para el bug de los dropdowns
      if (gigInfo.categories === '' || gigInfo.categories === 'Select a category') {
        showErrorToast('Please select a category');
        return;
      }
      
      if (gigInfo.expectedDelivery === '' || gigInfo.expectedDelivery === 'Expected delivery') {
        showErrorToast('Please select expected delivery');
        return;
      }
      ...
      console.log('GigInfo state:', gigInfo);
      ...
      if (isValid) {
        ...
        console.log('Creating gig with data:', gig);
        ...
        console.log('Validation failed for gigInfo:', gigInfo);
              ...
              <div className="relative col-span-4 md:w-11/12 lg:w-8/12">
                <Dropdown
                  ...
                  onClick={(item: string) => {
                    ...
                    setGigInfo((prev) => ({ ...prev, categories: item }));
                  }}
                />
              </div>
            </div>
            ...
            <div className="mb-12 grid md:grid-cols-5">
              ...
              <div className="relative col-span-4 md:w-11/12 lg:w-8/12">
                <Dropdown
                  ...
                  onClick={(item: string) => {
                    console.log('Expected delivery selected:', item);
                    setGigInfo((prev) => ({ ...prev, expectedDelivery: item }));
                  }}
                />
              </div>
            </div>
