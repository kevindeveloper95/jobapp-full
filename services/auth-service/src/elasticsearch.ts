...
async function checkConnection(): Promise<void> {
  ...
  while (!isConnected && retries < maxRetries) {
    ...
    try {
      ...
      retries++;
      log.error(`Connection to Elasticsearch failed. Retry ${retries}/${maxRetries}...`);
  ...
  if (!isConnected) {
    log.warn('⚠ Could not connect to Elasticsearch after 3 attempts. Auth service will start without Elasticsearch indexing.');
  }
}
